%% PALA_Beamforming.m : beamforming script for simulated RF data
...
myfilepath_RF = [workingdir filesep 'RF' filesep filename];
...
%% Load RF file
hhh=8;
load([myfilepath_RF '_RF'  num2str(hhh,'%03.0f') '.mat'],'RFdata','ListPos','Media','P');
...
myfilepath_IQ = [workingdir filesep 'IQ' filesep filename];
...
clb.Title.String='dB';
pp2 = plot(ListPos(:,1,1)*P.Wavelength,ListPos(:,3,1)*P.Wavelength,'rx','MarkerSize',10);title('Beamforming with Verasonics Vantage 4.4.1')
linkaxes(aa)
...
for ii = 1:1:size(Bmode,3)
    ...
    pp.XData = ListPos(:,1,ii)*P.Wavelength;pp.YData = ListPos(:,3,ii)*P.Wavelength;
    pp2.XData = ListPos(:,1,ii)*P.Wavelength;pp2.YData = ListPos(:,3,ii)*P.Wavelength;
    pause(.1)
end
